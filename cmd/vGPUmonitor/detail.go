...
const (
	CGROUP_ROOT  = "/sys/fs/cgroup/memory/kubepods.slice"
...
func getContainerUtilizationByUuid(containerName string, pod *v1.Pod, maDevUtil map[string]map[int]int) float64 {
...
func getPidsInContainer(pod *v1.Pod, containerID string) (map[int]struct{}, error) {
	...
	procFile := filepath.Join(baseDir, CGROUP_PROCS)
	return readProcsFile(procFile)
}

func getCgroupName(pod *v1.Pod, containerID string) string {
	podQos := pod.Status.QOSClass
	var parentContainer, prefix string
	switch podQos {
	case v1.PodQOSGuaranteed:
		parentContainer = CGROUP_ROOT
		prefix = guaranteed
	case v1.PodQOSBurstable:
		parentContainer = filepath.Join(CGROUP_ROOT, burstable+".slice")
		prefix = burstable
	case v1.PodQOSBestEffort:
		parentContainer = filepath.Join(CGROUP_ROOT, besteffort+".slice")
		prefix = besteffort
	default:
		return ""
	}
